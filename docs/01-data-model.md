...
create table attendance_records (
  ...
  review_note      text,                  -- the officer's words, shown back to the member
  member_note      text,                  -- the member's own, on a missing-credit request
  created_at       timestamptz not null default now(),
...
| `add_officer_attendance(event_id, member_ids[], submitted_value)` | files officer-entered attendance for one event and approves it, in one transaction, by handing the new ids to `review_records()`. Reads `event_categories.credit_mode` itself under a row lock and refuses a call whose `submitted_value` does not match it: a client holding an event row read minutes ago would otherwise file `null` against a `from_submission` link, which is approved credit worth zero and raises nothing. All or nothing, so a member who gained a live record in between (`PDS05`) does not leave the rest of the batch half-written. Refuses an archived or merged member (`PDS03`) |
| `add_officer_attendance_batch(event_id, entries, submitted_value)` | accepts a mixed, line-numbered event paste in one transaction and returns one structured outcome per input line. An entry explicitly linked to a live member enrolled for the event year is inserted as `officer_entry` and approved through `review_records()`. An entry explicitly left unmatched is inserted with `member_id = null`, its cleaned `claimed_name`, `source = 'officer_entry'`, `status = 'pending'`, and `unmatched_name`; it creates no member and cannot count. Invalid, repeated, and already-recorded entries create nothing. A live unmatched row with the same normalized name also counts as already recorded, even after that person joins the roster, so the preserved row must be linked and reviewed instead of receiving an approved duplicate. The function locks and rereads event credit configuration, rejects negative and numeric NaN values, and grants no anonymous access |
| `recover_officer_attendance_batch(event_id, batch_key)` | returns the committed per-line outcome stored by `add_officer_attendance_batch()` for the current authenticated officer, event, and opaque batch key. It is the authoritative read after a response is lost. It never attributes a row from an attendance snapshot to a batch and grants no anonymous access |
| `fn_retroactive_match_candidates(member_id)` | unresolved check-ins (`member_id` null, still `pending`) that might belong to this member, restricted to years they are enrolled in. `reason` is `exact_email` (an identity, from `fn_normalise_email()`) or `name_match` (a resemblance, trigram-scored on normalised names). Offered for an officer to confirm, never linked automatically. Refuses an archived member (`PDS03`) and follows a merged one to the survivor, carrying `resolved_member_id` and `followed_merge` on every row |
| `link_retroactive_matches(member_id, record_ids[])` | links exactly the given ids, the way `resolve_unmatched()` does for one record, or reports why not. Does not re-derive matches from the member id: only the ids the officer already confirmed are ever written. Returns one row per distinct requested id, `{record_id, outcome, resolved_member_id, followed_merge}`, `outcome` one of `linked`, `already_linked`, `not_pending`, `wrong_year`, `not_found`, `conflict`. Rechecks `pending` at write time, not only at preview time, so a record another officer rejected in between comes back `not_pending` rather than being linked on a stale screen. Not atomic: one refused id does not affect the others. Refuses an archived member (`PDS03`) and follows a merged one to the survivor. Does not approve: linked records stay `pending` for `review_records()` |
| `remove_attendance_record(record_id)` | deletes one record and, when it carries photos, writes a `purge_runs` row of kind `record_removed` naming their object paths, in the same transaction. Returns `{purge_run_id, object_paths}` for the caller to delete from the bucket and confirm with `finish_purge_run()`. The intent is recorded first because neither ordering is safe on its own: deleting the object first destroys a photo irreversibly if the row delete then fails, and deleting the row first strands bytes no operator tool can name, since `purge_orphaned_uploads()` only sees grants with `consumed_at IS NULL` and `submit_checkin()` consumes them |
...
reimplement the resolution: it runs `upsert_member_and_enroll()` over each row, so
the match tiers, the tombstone walk and the audit row have one implementation. It
returns one entry per input row in input order, carrying the caller's own CSV line
number, and a refused row carries the SQLSTATE and the message instead of a member
id.
